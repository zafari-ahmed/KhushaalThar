 ...
 */
class CompanyDeos extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('email_address', 'email','on'=>'insert'),
			array('user_type_id, status', 'numerical', 'integerOnly'=>true),
			array('password', 'match', 'pattern' => '/^.*(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).*$/', 'message'        => Yii::t('UsrModule.usr', 'Password must contain at least one lower and upper case character, a digit and minimum 6 character in length.')),
			array('password', 'match', 'pattern'=>'/\W/', 'message'=>'Password must contain at least one special character.'),
			array('password','length','min'=>6), 
			array('name, email_address, code, cnic, designation, mobile_number', 'length', 'max'=>255),
