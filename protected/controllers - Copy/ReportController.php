...
class ReportController extends Controller
{
    ...
    public function actionAlllabor($type=''){
        ini_set('max_execution_time', 50000);
        //$model = Labours::model()->findAll(array('limit'=>100));
        ...
        if($type=='pdf'){
            ...
            foreach($model as $m){
              ...
              $html .= '<tr><td>'.@$m->id.'</td><td>'.@$m->full_name.'</td><td>'.@$m->father_name.'</td><td>'.@$m->cnic.'</td><td>'.@$m->nicop.'</td><td>'.@$m->mobile_number.'</td><td>'.@$gender.'</td><td>'.($this->Laborwork(@$m->id)).'</td><td>'.(date('d M,Y',strtotime(@$m->createdOn))).'</td></tr>';
            }
        ...
        if($type=='excel'){
          ...
          $sheet1->setCellValue('BD3', 'Applicant Dumber Skill Test');


          
          
          $sheet1->getStyle("A3:BD3")->applyFromArray(array("font" => array( "bold" => true)));
          
          for($col = 'A'; $col !== 'BD'; $col++) {
          ...
          foreach($model as $m){
            ...
            $employments = $this->getRecentData($m->id,'employment');
            ...
            $educations = $this->getRecentData($m->id,'education');
            if(!empty($educations)){
